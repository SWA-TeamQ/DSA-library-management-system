...
class BookHashTable {
    struct Node { Book value; Node* next; Node(const Book& b, Node* n=nullptr): value(b), next(n) {} };
    vector<Node*> buckets;
    size_t sz{0};
...
public:
    ...
    bool insert(const Book& b) {
        ...
        while (cur) { if (cur->value.getISBN() == key) return false; cur = cur->next; }
    ...
    Book* find(const string& isbn) {
        ...
        for (Node* cur = buckets[idx]; cur; cur = cur->next)
            if (cur->value.getISBN() == isbn) return &cur->value;
        return nullptr;
    }

    bool erase(const string& isbn) {
        size_t idx = index(isbn);
        Node* cur = buckets[idx]; Node* prev = nullptr;
        while (cur) {
    ...
    vector<Book*> all() const {
        vector<Book*> out; out.reserve(sz);
        for (auto head : buckets) for (Node* cur=head; cur; cur=cur->next) out.push_back(&cur->value);
        return out;
    }
};
