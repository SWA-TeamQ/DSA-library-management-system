...
-   [ ] Fix controller add result: make [src/core/LibraryController.cpp](src/core/LibraryController.cpp) return the real BookManager::addBook result so duplicate ISBNs are reported correctly.
-   [ ] Add book update flow (title/author/category/edition/year/availability) and menu entry; reuse controller.updateBook.
-   [ ] Guard removals: ensure BookManager remove blocks active loans via LoanService/TransactionManager check; surface messaging in UI.
